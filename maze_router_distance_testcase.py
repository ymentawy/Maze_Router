# -*- coding: utf-8 -*-
"""Maze_router_distance_testcase.ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/15gb4fqnetM-rDntcrahSsX59i7nuAXKS
"""
...
class LeeRouter:
    ...
    def route_net(self, net_name: str, pins: List[Tuple[int, int, int]],
                  existing_routes: List[List[Tuple[int, int, int]]] = None) -> Tuple[List[Tuple[int, int, int]], float]:
        if len(pins) < 2:
        ...
        for i in range(len(adjusted_pins) - 1):
            ...
            # Pass existing routes to the routing algorithm
            path, cost = self._lee_route_with_congestion(
    ...
    def _lee_route_with_congestion(self, start_layer: int, start_x: int, start_y: int,
                                   end_layer: int, end_x: int, end_y: int,
                                   existing_routes: List[List[Tuple[int, int, int]]],
                                   congestion_penalty: float) -> Tuple[List[Tuple[int, int, int]], float]:
        directions = [
        ...
        # Priority Queue for routing with congestion awareness
        queue = PriorityQueue()
        ...
        while not queue.empty():
            ...
            for dx, dy, dlayer in directions:
                ...
                # Add congestion penalty from routing density
                congestion_factor = 1 + self.routing_density[new_layer][new_y, new_x]
...
def net_priority(net: Tuple[str, List[Tuple[int, int, int]]]) -> Tuple[int, float, float]:
    net_name, pins = net
    num_pins = len(pins)
    ...
    if layer_coords[0] and layer_coords[1]:
    ...
    priority_score = (
        -num_pins,           # Favor nets with more pins
...
def main():
    """Demonstrate routing with and without net priority based on Manhattan distance"""
    print("Running routing simulation to show net priority impact based on Manhattan distance...")
    ...
    # Obstacle configuration (same as before)
    obstacles = [
    ...
    # Nets with the same number of pins but different Manhattan distances
    ...
    # Second run with priority based on Manhattan distance
    print("\n--- SECOND RUN: ROUTING WITH NET PRIORITY (Manhattan distance) ---")
    ...
    # Sort nets by priority (using the provided net_priority function)
